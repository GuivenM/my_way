...
import { getEntry, saveEntry, getEntries } from '../api'
...
export default function Journal() {
  ...
  useEffect(() => {
    setSaved(true)
    getEntry(date).then(d => setContent(d.content || '')).catch(() => setContent(''))
  }, [date])

  useEffect(() => {
    if (tab === 'list') getEntries(30).then(d => setEntries(d.entries || []))
  }, [tab])

  const doSave = useCallback(async (text) => {
    setSaving(true)
    try { await saveEntry(date, text); setSaved(true) } finally { setSaving(false) }
  }, [date])
